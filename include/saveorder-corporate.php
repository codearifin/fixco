...
if(isset($_POST['submit'])){
		...
		if($TokenIdform==1):
				 ...
				 //select member
			 	 ...
			 	 if(isset($_SESSION['user_status'])): $Userstatus = $_SESSION['user_status']; else: $Userstatus ='';endif; //Added by Andy on 16 May 2017 - Req by coh Adri 
			 	 //Apabila user biasa order, superuser juga menerima email
			 	 if($Userstatus == "SUPER USER"):
					 ...
					 $totaldeposit = gettotaldepositcorporate($idmember);
			 	 	...
			 	 	$qummep = $db->query("SELECT `id`,`idmember_list`,`email` FROM `corporate_user` WHERE `id`='$Usertokenid'");
					 $ros = $qummep->fetch_assoc();	
					 $uidmembermain = $ros['idmember_list']; $iduser = $ros['id']; $emailmember = $ros['email'];

					 $query = $db->query("SELECT * FROM `member` WHERE `id`='$uidmembermain'");
					 $res = $query->fetch_assoc();
					 $idmember = $res['id'];
					 $emailmaster = $res['email'];
					 $membername = $res['name'].' '.$res['lastname'];
					 $totaldeposit = gettotaldepositcorporate($idmember);
			 	 endif;
