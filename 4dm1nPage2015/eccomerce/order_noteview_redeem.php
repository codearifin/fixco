<?php
	...
	$que = $db->query("SELECT `id`,`date` FROM `order_header_redeemlist` WHERE `id`='$idorder'");
	$row = $que->fetch_assoc();
?>
<style>
...
<script type="text/javascript">
		...
		$('form#add_listform').validate({
		  ...
		  errorPlacement: function(error, element){
		   ...
		   submitHandler:function(form){
				  ...
				  $.ajax({
					  url:'eccomerce/dosave_datalistnote_redeem.php',
					  type:'POST',
					  data:formData,
					  dataType:'HTML',
					  contentType:false,
	...
	$("a.delete_listnote").click(function(e) {
		...
		$.post("eccomerce/delete_noteorder_redeem.php", {"idpage": idpage},
	    function(data) {
			if(data==1){
...
<div id="order-detail">
	<h2 class="f-gm">Redeem Order ID <span><?php echo sprintf('%06d',$row['id']).'';?></span></h2>
    ...
    <div class="od-bottom">
    	<div class="odb-top">
            <h2 class="f-mb">Detail Status</h2>
				...
				<?php
                        $pagestatus = 0;
                        $quee3 = $db->query("SELECT `id`,DATE_FORMAT(`date`,'%d %M %Y %H:%i:%s') as tgl,`description` FROM `status_detailorder_redeem` WHERE `idorder`='$idorder' ORDER BY `date` DESC") or die(mysql_error());
