<?php 
	...
	$query = $db->query("SELECT `id` FROM `corporate_user` WHERE `email`='$email' and `status_active` = 1 ");
	...
	if($jumpage > 0):
					...
					$query2 = $db->query("UPDATE `corporate_user` SET `password` = '$passwordNew' WHERE `email` = '$email' ");
